			...
			this.statechange = function(response,PQuerikID){
				if(response.readyState == 4 && response.status == 200){
					...
					if(data.results){
						...
						var childObjects = new Array(data.results.length); // Initialize the array object
						alert(childObjects.length);
						for(var i = 0; i < childObjects.length; i++)
						{
							...
							for(var key in data.results[i]){
								 // Get the header text
								textNode = document.createTextNode(data.results[i][key]);
								...
								childObjects[i].appendChild(textNode); // Append text to the object							
							}
						}
						document.getElementById(targetName).appendChild(childObjects);
					}
